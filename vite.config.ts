...
// Custom plugin: serve `*.mdx?raw` as the raw file contents, bypassing the
// MDX compiler so `import.meta.glob('*.mdx', { query: '?raw' })` works.
// Plan 02-03 — needed for accurate wordCount/readingTime measurement from
// the full body source (frontmatter + import-line + JSX-tag stripping is
// performed in src/lib/posts.ts).
